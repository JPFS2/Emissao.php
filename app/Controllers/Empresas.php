...
class Empresas extends Controller
{
    ...
    public function show($id_empresa)
    {
        ...
        $data['links_adicionais_da_sidebar'] = $this->link_adicional_da_sidebar_model
    ...
    {        
        ...
        // Verifica se a empresa é isenta ou não
        if($dados['isento'] == 1):
            $dados['IE'] = "";
        endif;
        ...
        if(isset($dados['id_login'])) :
            ...
            // Altera somente se o plano escolhido for diferente do que já esta cadastrado
            // Assim não afeta o controle de acesso dos usuários, caso queira mudar outra informação da empresa
            $id_plano_atual_da_empresa = $this->empresa_model
                                                        ->where('id_empresa', $dados['id_empresa'])
                                                        ->select('id_plano')
                                                        ->first()['id_plano'];            

            if($id_plano_atual_da_empresa != $dados['id_plano']): // Se for diferente então escolheu outro plano
                $controle_de_acesso_do_plano = $this->controle_de_acesso_model
                                                                    ->where('id_plano', $dados['id_plano'])
                                                                    ->first();

                // remove id_controle_de_acesso para não dar conflito
                unset($controle_de_acesso_do_plano['id_controle_de_acesso']);

                // Atualiza o controle de acesso do usuario com o controle de acesso do plano escolhido
                // Isso afeta tanto os controle de acesso das empresas quanto de todos os usuários relacionados a essa empresa
                $this->controle_de_acesso_do_usuario_model
                                    ->where('id_empresa', $dados['id_empresa'])
                                    ->set($controle_de_acesso_do_plano)
                                    ->update();
            endif;
            ...
            return redirect()->to(base_url("/empresas/edit/{$dados['id_empresa']}"));
        ...
        // Calcula de acordo com o plano a data de encerramento do contrato
        $plano = $this->plano_model
                            ->where('id_plano', $dados['id_plano'])
                            ->first();

        $data_de_encerramento_do_contrato = date('Y-m-d', strtotime("+{$plano['duracao']} days", strtotime(date('Y-m-d'))));

        $dados['data_de_encerramento_do_contrato'] = $data_de_encerramento_do_contrato;
        ...
        $controle_de_acesso = $this->controle_de_acesso_model
                                                    ->where('id_plano', $dados['id_plano'])
                                                    ->first();

        // Remove id_controle_de_acesso para não dar conflito
        unset($controle_de_acesso['id_controle_de_acesso']);

        $controle_de_acesso['id_empresa'] = $id_empresa;
        $controle_de_acesso['id_login'] = $id_login;

        $this->controle_de_acesso_do_usuario_model
                                        ->insert($controle_de_acesso);
    ...
    public function createLinkAdicionalDaSidebar($id_empresa)
    {
        if($this->session->get('tipo') != $this->tipo) :
            $this->session->setFlashdata(
                'alert', 
        ...
        echo view('templates/header', $data);
        echo view('empresas/form_link_adicional');
        echo view('templates/footer');
    }

    public function editLinkAdicionalDaSidebar($id_link, $id_empresa)
    {
        if($this->session->get('tipo') != $this->tipo) :
            $this->session->setFlashdata(
                'alert', 
        ...
        $data['link_da_sidebar'] = $this->link_adicional_da_sidebar_model
                                                ->where('id_empresa', $id_empresa)
    ...
    public function storeLinkAdicionalDaSidebar()
    {
        $dados = $this->request->getVar();

        if(isset($dados['id_link'])):
            $this->link_adicional_da_sidebar_model
                                    ->where('id_empresa', $dados['id_empresa'])
                                    ->where('id_link', $dados['id_link'])
                                    ->set($dados)
                                    ->update();
        ...
        return redirect()->to(base_url("empresas/editLinkAdicionalDaSidebar/{$dados['id_link']}/{$dados['id_empresa']}"));
        endif;
        ...
        return redirect()->to(base_url("empresas/show/{$dados['id_empresa']}"));
    }

    public function deleteLinkAdicionalDaSidebar($id_link, $id_empresa)
    {
        // Apaga primeiro a empresa
        $this->link_adicional_da_sidebar_model
            ->where('id_empresa', $id_empresa)
        ...
        $this->session->setFlashdata(
            ...
            [
                'type'  => 'success',
                'title' => 'Link excluido com sucesso!'
            ]
        );

        return redirect()->to(base_url("empresas/show/$id_empresa"));
    }
}