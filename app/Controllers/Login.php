...
class Login extends Controller
{
    ...
    public function autenticar()
    {
        ...
        if(!empty($login)) :
            ...
            // Verifica se a empresa é ATIVA
            if($login['tipo'] == 2):
                $empresa = $this->empresa_model
                                ->where('id_empresa', $login['id_empresa'])
                                ->select('xFant, logomarca, status')
                                ->first();

                if($empresa['status'] == "Desativado"):
                    $this->session
                        ->setFlashdata(
                            'alert',
                            [
                                'type' => 'error',
                                'title' => 'Empresa não possui permissão para acessar o sistema. Por favor, entrar em contato com o administrador!'
                            ]
                        );
            ...
            if($login['tipo'] == 1) : // VERIFICAR SE A LÓGICA ESTÁ CORRETA
            ...
            elseif($login['tipo'] == 2) :
                ...
                // Alerta de sucesso na autenticação 
                $this->session
                    ->setFlashdata(
                        'alert',
                        [
    ...
    public function usuarios()
    {
        ...
        $permissao = $this->controle_de_acesso_do_usuario_model->verificaPermissao('usuarios');
        ...
        $data['controle_de_acesso'] = $this->controle_de_acesso_do_usuario_model
                                                        ->where('id_empresa', $this->id_empresa)
    ...
    public function create()
    {
        ...
        $data['controle_de_acesso_do_plano'] = $this->controle_de_acesso_model
                                                        ->where('id_plano', $empresa['id_plano'])
                                                        ->first();
    ...
    public function edit($id_login)
    {
        ...
        $data['controle_de_acesso_do_usuario_selecionado'] = $this->controle_de_acesso_do_usuario_model
                                                                                ->where('id_empresa', $this->id_empresa)
                                                                                ->where('id_login', $id_login)
                                                                                ->first();
    ...
    public function store()
    {
        ...
        if(isset($dados['id_login'])):
            ...
            return redirect()->to(base_url("/login/edit/{$dados['id_login']}"));
        ...
        return redirect()->to(base_url('/login/usuarios'));
    }
