...
class Orcamentos extends Controller
{
    ...
    public function show($id_orcamento)
    {
        ...
        $data['formas_de_pagamento'] = $this->forma_de_pagamento_do_orcamento_model
                                                            ->where('id_empresa', $this->id_empresa)
                                                            ->where('id_orcamento', $id_orcamento)
                                                            ->join(
                                                                'formas_de_pagamento',
                                                                'formas_de_pagamento_do_orcamento.id_forma = formas_de_pagamento.id_forma'
                                                            )
                                                            ->findAll();
    ...
    public function finalizarVenda($id_orcamento)
    {
        ...
        // ------------------------ PASSA AS FORMAS DE PAGAMENTO DO ORÇAMENTO PARA A VENDA ---------------------- //
        $formas_de_pagamento = $this->forma_de_pagamento_do_orcamento_model
                                                                ->where('id_empresa', $this->id_empresa)
                                                                ->where('id_orcamento', $id_orcamento)
                                                                ->findAll();
