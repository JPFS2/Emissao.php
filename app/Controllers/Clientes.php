...
class Clientes extends Controller
{
    ...
    public function show($id_cliente)
    {
        ...
        // $data['pagamentos'] = $this->pagamento_do_cliente_model
        ...
        $data['creditos_na_loja'] = $this->credito_na_loja_model
                                        ->where('id_empresa', $this->id_empresa)
                                        ->where('id_cliente', $id_cliente)
                                        ->findAll();

        $data['vendas_credito_na_loja'] = $this->venda_credito_na_loja_model
                                                        ->where('id_empresa', $this->id_empresa)
                                                        ->where('id_cliente', $id_cliente)
                                                        ->findAll();

        // Totais necessários
        $somatorio_de_credito_na_loja = $this->credito_na_loja_model
                                                                ->where('id_empresa', $this->id_empresa)
                                                                ->where('id_cliente', $id_cliente)
                                                                ->selectSum('valor')
                                                                ->first()['valor'];

        $somatorio_de_vendas_credito_na_loja = $this->venda_credito_na_loja_model
                                                                    ->where('id_empresa', $this->id_empresa)
                                                                    ->where('id_cliente', $id_cliente)
                                                                    ->selectSum('valor_a_pagar')
                                                                    ->first()['valor_a_pagar'];

        $restante = $somatorio_de_credito_na_loja - $somatorio_de_vendas_credito_na_loja;

        $ordens_de_servicos = $this->ordem_de_servico_model
                                                        ->where('ordens_de_servicos.id_empresa', $this->id_empresa)
                                                        ->where('ordens_de_servicos.id_cliente', $id_cliente)
                                                        ->join('clientes', 'ordens_de_servicos.id_cliente = clientes.id_cliente')
                                                        ->findAll();

        $ordens_de_servicos_laboratorio = $this->laboratorio_definitivo_model
                                                        ->where('laboratorios.id_empresa', $this->id_empresa)
                                                        ->where('laboratorios.id_cliente', $id_cliente)
                                                        ->join('clientes', 'laboratorios.id_cliente = clientes.id_cliente')
                                                        ->findAll();

        $data['somatorio_de_credito_na_loja'] = $somatorio_de_credito_na_loja;
        $data['somatorio_de_vendas_credito_na_loja'] = $somatorio_de_vendas_credito_na_loja;
        $data['restante'] = $restante;

        $data['ordens'] = $ordens_de_servicos;
        $data['ordens_lab'] = $ordens_de_servicos_laboratorio;
    ...
    public function createCreditoNaLoja($id_cliente)
    {
        // Verifica se tem permissão de acesso
        if ($url = $this->permissao):
            return redirect()->to($url);
        ...
        echo view('templates/header', $data);
        echo view('clientes/form_credito_na_loja');
        echo view('templates/footer');
    }

    public function storeCreditoNaLoja($id_cliente)
    {
        $dados = $this->request->getVar();
        ...
        // Adiciona o que falta ao array
        $dados['data']       = date('Y-m-d');
        $dados['hora']       = date('H:i:s');
        $dados['id_cliente'] = $id_cliente;
        $dados['id_empresa'] = $this->id_empresa;

        $this->credito_na_loja_model
                        ->insert($dados);
        ...
        $this->session
            ->setFlashdata(
                ...
                [
                    'type'  => 'success',
                    'title' => 'Crédito na loja adicionado com sucesso!'
                ]
            );

        return redirect()->to(base_url("clientes/show/{$id_cliente}"));
    }

    public function deleteCreditoNaLoja($id_credito, $id_cliente)
    {
        // Verifica se tem permissão de acesso
        if ($url = $this->permissao):
            return redirect()->to($url);
        ...
        $this->session->setFlashdata(
            ...
            [
                'type'  => 'success',
                'title' => 'Crédito excluido com sucesso!'
            ]
        );
    ...
    public function deleteVendaCreditoNaLoja($id_venda_credito_na_loja, $id_cliente)
    {
        // Verifica se tem permissão de acesso
        if ($url = $this->permissao):
            return redirect()->to($url);
        ...
        $this->venda_credito_na_loja_model
            ->where('id_empresa', $this->id_empresa)
            ->where('id_cliente', $id_cliente)
            ->where('id_venda_credito_na_loja', $id_venda_credito_na_loja)
            ->delete();
