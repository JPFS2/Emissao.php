...
class Relatorios extends Controller
{
    ...
    public function lucroDasVendas()
    {
        // Verifica se tem permissão de acesso
        $permissao = $this->controle_de_acesso_model->verificaPermissao('vendas_lucro_total');
        ...
        $data['titulo'] = [
            'modulo' => 'RELATÓRIO DE LUCROS',
            'icone'  => 'fa fa-list'
        ];

        $data['caminhos'] = [
            ['titulo' => "Início", 'rota' => "/inicio", 'active' => false],
            ['titulo' => "Relatório de Lucros", 'rota'   => "", 'active' => true]
        ];
        ...
        $total_de_produtos_vendidos = $this->produto_da_venda_model
                                                            ->where('produtos_da_venda.id_empresa', $this->id_empresa)
                                                            ->join('vendas', 'produtos_da_venda.id_venda = vendas.id_venda')
                                                            ->where('data >=', $dados['data_inicio'])
                                                            ->where('data <=', $dados['data_final'])
                                                            ->countAllResults();
        ...
        foreach($produtos_da_venda as $produto):
            $lucro_total += (($produto['quantidade'] * $produto['lucro']) - $produto['desconto']);
        endforeach;
                                        
        $data['total_de_vendas'] = $total_de_vendas;
        $data['total_de_produtos_vendidos'] = $total_de_produtos_vendidos;
        $data['lucro_total'] = $lucro_total;
        ...
        echo view('templates/header', $data);
        echo view('relatorios/vendas/lucro_das_vendas');
        echo view('templates/footer');
    }
}
